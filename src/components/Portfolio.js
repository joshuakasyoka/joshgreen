...
import emmaCaseStudy from '../data/caseStudies/emma';
import moataGeospatialCaseStudy from '../data/caseStudies/moataGeospatial';
import { PRESENTABLE_CASE_STUDIES } from '../data/caseStudies';
import winRoomCaseStudy from '../data/caseStudies/winRoom';
import monthlyProjectReviewCaseStudy from '../data/caseStudies/monthlyProjectReview';
import { MPR_DIAGRAMS } from './MprDiagrams';
import { MPR_APP_DEMOS } from './MprAppDemos';
import { WIN_ROOM_DIAGRAMS } from './WinRoomDiagrams';
import { CaseStudyLockRow } from './CaseStudyLockPrompt';
import { isLocked, isUnlocked } from '../utils/caseStudyLock';
import artefactsCaseStudy from '../data/caseStudies/artefacts';
...
import { PRESENTATION_DEMOS } from './presentation/demoRegistry';
import { EmmaLibraryDiagram } from './CaseStudyDiagrams';
...
// Project id → /present/:slug, for case studies that have a presentation.
const PRESENTATION_SLUG_BY_ID = Object.fromEntries(
  Object.entries(PRESENTABLE_CASE_STUDIES).map(([slug, study]) => [study.id, slug])
);
...
const Portfolio = ({ isDarkMode, toggleDarkMode, initialProjectId = null }) => {
  const [projects] = useState({
    'AI Exploration': [
      {
        ...
        website: 'https://apps.apple.com/gb/app/bug-club/id6788859354',
    ...
    'Product Design': [
      artefactsCaseStudy,
      moataGeospatialCaseStudy,
      emmaCaseStudy,
      monthlyProjectReviewCaseStudy,
      winRoomCaseStudy,
      {
        id: 16,
        name: 'Moata 3D',
  ...
  const renderProjectMedia = (imgObj, project, idx, { maxWidth = '92%', maxHeight = '65vh', openModal }) => {
    ...
    if (demo === 'emma-chat-exchange') {
      demoElement = <EmmaChatExchangeDemo className="self-start w-full" />;
    }
    if (demo === 'emma-insight-graph') {
      demoElement = <EmmaInsightGraphDemo className="self-start w-full" />;
    }
    if (demo === 'emma-library-diagram') {
      demoElement = <EmmaLibraryDiagram className="self-start w-full" />;
    }
    if (WIN_ROOM_DIAGRAMS[demo] || MPR_DIAGRAMS[demo] || MPR_APP_DEMOS[demo]) {
      const Diagram = WIN_ROOM_DIAGRAMS[demo] || MPR_DIAGRAMS[demo] || MPR_APP_DEMOS[demo];
      demoElement = <Diagram className="self-start w-full" />;
    }
    if (!demoElement && demo && PRESENTATION_DEMOS[demo]) {
      const Demo = PRESENTATION_DEMOS[demo];
      demoElement = <Demo className="self-start w-full" />;
    }
    if (demo === 'emma-artefact-panel') {
      demoElement = <EmmaArtefactPanelDemo className="self-start w-full" />;
    }
    if (demo === 'emma-artefact-open-close') {
      demoElement = <EmmaArtefactOpenCloseDemo className="self-start w-full" />;
    }
    if (demo === 'emma-artefact-sketch') {
      demoElement = <EmmaArtefactSketchDemo className="self-start w-full" />;
    }
    if (demo === 'emma-artefact-table') {
      demoElement = <EmmaArtefactTableDemo className="self-start w-full" />;
    }
  ...
  const orderedCategories = useMemo(() => {
    ...
    const priority = ['AI Exploration', 'Product Design', 'Participatory Design', 'Design Writing', 'Older Work'];
  ...
  const handleProjectClick = (project) => {
    // A private case study asks for its passphrase in the row itself.
    if (isLocked(project.id) && !isUnlocked(project.id) && !unlockedIds.includes(project.id)) {
  ...
  const unlockProject = (project) => {
    setUnlockedIds((ids) => Array.from(new Set([...ids, project.id])));
    setUnlockingId(null);
    setSelectedProject(project);
  };

  const projectIsLocked = (project) =>
    Boolean(project) && isLocked(project.id) && !isUnlocked(project.id) && !unlockedIds.includes(project.id);
                    ...
                    <React.Fragment key={project.id}>
                      ...
                      >
                        <div className="flex items-center justify-between gap-3 pr-3">
                          ...
                          >
                            {unlockingId === project.id ? (
                              <CaseStudyLockRow
                                ...
                                onUnlocked={() => unlockProject(project)}
                            ...
                            {project.date && unlockingId !== project.id && (
                          ...
                          ) : isLocked(project.id) && !isUnlocked(project.id) && !unlockedIds.includes(project.id) ? (
                            <span
                              key={`locked-${project.id}`}
                              className={`portfolio-lock-dot transition-opacity duration-300 ${
                                selectedProject?.id === project.id ? 'opacity-100' : 'opacity-45'
                              }`}
                              title="Private - passphrase required"
                            >
                              <Lock size={13} strokeWidth={1.9} />
                            </span>
                          ) : selectedProject?.id === project.id && PRESENTATION_SLUG_BY_ID[project.id] ? (
                            // The active dot doubles as the way into present mode:
                            // hovering turns it into a play icon.
                            <Link
                              key={`active-${project.id}`}
                              to={`/present/${PRESENTATION_SLUG_BY_ID[project.id]}`}
                              onClick={(event) => event.stopPropagation()}
                              className="portfolio-present-dot custom-clickable"
                              aria-label={`Present ${project.name}`}
                              title="Present"
                            >
                              <span className="portfolio-present-dot__dot portfolio-active-dot" />
                              <svg className="portfolio-present-dot__play" viewBox="0 0 10 10" aria-hidden="true">
                                <path d="M2.5 1.2 L8.8 5 L2.5 8.8 Z" fill="#81FF03" stroke="#81FF03" strokeWidth="1" strokeLinejoin="round" />
                              </svg>
                            </Link>
                          ) : (
                            <div
                      ...
                      {project.images && project.images.length > 0 && project.id === selectedProject?.id && !projectIsLocked(project) && (
